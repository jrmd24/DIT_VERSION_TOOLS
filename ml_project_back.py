...
def process_client_request(
    ...
    """Traite la requête d'un client et lui génère un modèle ML suivant le type demandé ainsi que les résultats d'évaluation"""
    model_file_path = Path("")
    model_eval_results_dict = {
        "rmse": "0",
        "accuracy": "0",
        "confusionMatrix": Path(""),
    }
    return client_id, model_file_path, model_eval_results_dict
...
def preprocess_data(df, request_id):
    ...
    save_artifact(
        preprocessed_data, DATA_PATH.parent / "preprocessed_data.csv", request_id
    )
...
def build_model(request_id):
    ...
    logger.info("Construction du modèle", extra={"request_id": request_id})
...
def evaluate_model(model, X_test, y_test, request_id):
    ...
    ax[1].plot(
        fpr, tpr, color="darkorange", lw=2, label=f'AUC = {metrics["roc_auc"]:.2f}'
    )
...
def process_request():
    ...
    try:
        ...
        model_path = save_artifact(
            model, MODEL_DIR / "fraud_detection_model.pkl", request_id
        )
        ...
        logger.info(f"Traitement réussi {request_id}", extra={"request_id": request_id})
