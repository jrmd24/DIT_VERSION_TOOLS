...
def load_data(file_path, request_id):
    ...
    try:
        ...
        logger.error(f"Erreur de chargement: {str(e)}", extra={"request_id": request_id})
        raise
...
def preprocess_data(df, request_id, target_col, model_type):
    ...
    save_artifact(preprocessed_data, DATA_DIR / f"preprocessed_data_{request_id}.csv", request_id)
...
def build_model(model_name, model_type):
    ...
    return Pipeline([
        ...
        ('model', model_map[model_type].get(model_name, 
            LogisticRegression() if model_type == "classification" 
